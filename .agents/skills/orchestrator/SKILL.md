...
Codex's shell CWD can silently drift into a worktree path after agent
...
> **No PreToolUse guard in Codex:** Unlike Claude Code, Codex has no
...
4. After cleanup, escalate to the user: "Shell CWD is unrecoverable. Please
   restart Codex from PROJECT_ROOT."
...
# Clean up epic branch (remote FIRST, then local)
...
worktrees manually on the story branch.
...
### Branch Locking
Git prevents two worktrees from checking out the same branch. Dev worktree MUST be
removed before PM worktree can be created on the same story branch.
