...
name: ba_challenger
description: >
  Adversarially reviews BUSINESS.md after the BA produces it. Only spawned when
  dnf.specialist_review is enabled. Checks for OMISSIONS, HALLUCINATIONS, and DRIFT
  against user-provided context. Returns REVIEW_RESULT (APPROVED/REJECTED) with
...
```bash
codex "Use skill ba_challenger. Review BUSINESS.md against the user requirements."
```
